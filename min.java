...
public class min {
	...
	// get the min value from object.salary and find the minimum salary
	// if we give the String i will give results based on ASCII code  value
	// comparator order is reverse or natural it will give the minimum value
	System.err.println(list.stream().map(Employee::getName).peek(System.out::println).min(Comparator.naturalOrder()));
	}
}
